...
int main(int argc, char *argv[]) //Paranmeter of this method requires the path to where the .txt file is located. This is evident when running show.c in the terminal when the
{
  ...
  while((Bytes_Read = read(FD, Array_MB, Max_Bytes)) > 0){ //if there is actual content to read, the while loop will keep readung until it hits zero it'll never be -1 unless there is nothing to read
    //write(file_descriptor, string *buffer, int num_chars_to_write). This system call pushes bytes from our memory buffer (array type char where the data is stored) and writes/prints it into the terminal
    if(write(STDOUT_FILENO, Array_MB, Bytes_Read) != Bytes_Read){ //for the first argument of write, dont use the FD, this will simply re-wriote the contents from memory buffer or array back to the original file rather than the terminal
      perror("error when writing to the terminal");
      close(FD);
    }
  }
  if(Bytes_Read == -1){
